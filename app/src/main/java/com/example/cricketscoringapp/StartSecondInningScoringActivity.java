...
public class StartSecondInningScoringActivity extends AppCompatActivity {
TextView battingTeamNameTextView, bowlingTeamNameTextView,targetTxt,scoreTxt,totalOversTxt,strikerTxt,nonStrikerTxt,requiredTxt,currentBowlerTxt;

    private String matchId, setBattingTeamName,setBowlingTeamName;
    private DatabaseReference matchRef;
    private String currentStriker, currentNonStriker, currentBowler, battingTeam, bowlingTeam;
    private int totalRuns = 0, totalWickets = 0, totalBalls = 0, currentOver = 0,target;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_start_second_inning_scoring);
        ...
        battingTeamNameTextView = findViewById(R.id.battingTeamTextView2ndInning);
                bowlingTeamNameTextView=findViewById(R.id.bowlingTeamNamet2ndInning);
        targetTxt=findViewById(R.id.targetTextview2ndInning);
                scoreTxt=findViewById(R.id.currentScoreTextView2ndInning);
        totalOversTxt=findViewById(R.id.totalOversTxt2ndInning);
                strikerTxt=findViewById(R.id.strikerTextView2ndInning);
        nonStrikerTxt=findViewById(R.id.nonStrikerTextView2ndInning);
                requiredTxt=findViewById(R.id.requiredRunsTextView2ndInning);
        currentBowlerTxt=findViewById(R.id.bowlerTextView2ndInning);
        swappingButton = findViewById(R.id.swapButton2ndInning);
        recyclerView = findViewById(R.id.bowlsRecyclerview2ndInning);


        recyclerView.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false));

        matchId = getIntent().getStringExtra("matchId");
        matchRef = FirebaseDatabase.getInstance().getReference("matches").child(matchId).child("scorecard").child("secondInning");
    ...
    private void loadMatchData() {
        matchRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (currentStriker != null && currentNonStriker != null && currentBowler != null && battingTeam != null && bowlingTeam != null) {
                    strikerTxt.setText("Striker: " + currentStriker);
                    nonStrikerTxt.setText("Non-Striker: " + currentNonStriker);
                    currentBowlerTxt.setText("Bowler: " + currentBowler);

                    userRef.child("teams").child(battingTeam).addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            Team team = snapshot.getValue(Team.class);
                             setBattingTeamName = team.getName();

                            battingTeamNameTextView.setText("Batting Team: " + setBattingTeamName);
                        }
                    ...
                    userRef.child("teams").child(bowlingTeam).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            Team team = snapshot.getValue(Team.class);
                             setBowlingTeamName = team.getName();
                    ...
                    Toast.makeText(StartSecondInningScoringActivity.this, "Failed to load match data", Toast.LENGTH_SHORT).show();
                }
            }
        ...
        totalOversstaticRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                totalOversTxt.setText("("+dynamicOversOfMatch +"/"+staticOversOfMatch+")");
            }
...
FirebaseDatabase.getInstance().getReference("matches").child(matchId).child("scorecard").child("firstInning").child("totalRuns").addListenerForSingleValueEvent(new ValueEventListener() {
    @Override
    public void onDataChange(@NonNull DataSnapshot snapshot) {
        int totalRunsFromdbforTarget = snapshot.getValue(Integer.class);
        target = totalRunsFromdbforTarget + 1;
        targetTxt.setText(String.valueOf(target));
    }
...
FirebaseDatabase.getInstance().getReference("matches").child(matchId).addListenerForSingleValueEvent(new ValueEventListener() {
    @Override
    public void onDataChange(@NonNull DataSnapshot snapshot) {
        Match match = snapshot.getValue(Match.class);
        double totalOversFromDb = match.getOvers();
      int  totalBallsforSet = (int) (totalOversFromDb * 6);

      int remainingBalls = totalBallsforSet - totalBalls;

      requiredTxt.setText(battingTeam+"Required "+(target-totalRuns) +"Runs in" +remainingBalls);
    ...
    private void setButtonListeners() {
        findViewById(R.id.runButton02ndInning).setOnClickListener(v -> updateScore(0, false));
        findViewById(R.id.runButton12ndInning).setOnClickListener(v -> updateScore(1, false));
        findViewById(R.id.runButton22ndInning).setOnClickListener(v -> updateScore(2, false));
        findViewById(R.id.runButton32ndInning).setOnClickListener(v -> updateScore(3, false));
        findViewById(R.id.runButton42ndInning).setOnClickListener(v -> updateScore(4, false));
        findViewById(R.id.runButton52ndInning).setOnClickListener(v -> updateScore(5, false));
        findViewById(R.id.runButton62ndInning).setOnClickListener(v -> updateScore(6, false));

        findViewById(R.id.wideButton2ndInning).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                extraType = "Wide";
        ...
        findViewById(R.id.noBallButton2ndInning).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                extraType="No";
                showExtraRunsDialog(true);
        ...
        findViewById(R.id.wicketBallButton2ndInning).setOnClickListener(v -> showWicketDialog());
...
// Update the display of total overs correctly
        ...
        scoreTxt.setText("Score: " + totalRuns + "/" + totalWickets);

        totalOversstaticRef.child("scorecard").child("secondInning").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
    ...
    private void checkEndOfInning() {
        if (totalWickets == 10 || dynamicOversOfMatch >= staticOversOfMatch || totalRuns >= target) {
    ...
    private void showEndOfInningDialog() {
        ...
        if (totalWickets == 10 && totalRuns < target){
            builder.setMessage(bowlingTeam +"won by"+ (target - totalRuns)+"Runs" );
            FirebaseDatabase.getInstance().getReference("matches").child(matchId).child("status").setValue("Completed");
            FirebaseDatabase.getInstance().getReference("matches").child(matchId).child("wonStatus").setValue(setBowlingTeamName +" won by "+ (target - totalRuns)+" Runs ");
        } else if (dynamicOversOfMatch >= staticOversOfMatch && totalRuns < target) {
            builder.setMessage(bowlingTeam +"won by"+ (target - totalRuns)+"Runs");
            FirebaseDatabase.getInstance().getReference("matches").child(matchId).child("status").setValue("Completed");
            FirebaseDatabase.getInstance().getReference("matches").child(matchId).child("wonStatus").setValue(setBowlingTeamName +" won by "+ (target - totalRuns)+" Runs ");
        } else if ( totalRuns >= target) {
            builder.setMessage(battingTeam + "won by "+(10-totalWickets)+"Wickets");
            FirebaseDatabase.getInstance().getReference("matches").child(matchId).child("status").setValue("Completed");
            FirebaseDatabase.getInstance().getReference("matches").child(matchId).child("wonStatus").setValue(setBattingTeamName +" won by "+ (10 - totalWickets)+" Wickets ");
        }
        ...
        builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                Intent intent = new Intent(StartSecondInningScoringActivity.this, MainActivity.class);
    ...
    private void changeBowler() {
        ...
        bowlingTeamRef.addValueEventListener(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(StartSecondInningScoringActivity.this, "Failed to load bowlers", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void recordWicket(String playerOut, String type, int runs) {
        ...
        if (totalWickets != 10){
            ...
            batsmansTeamRef.addValueEventListener(new ValueEventListener() {
                ...
                @Override
                public void onCancelled(@NonNull DatabaseError error) {
                    Toast.makeText(StartSecondInningScoringActivity.this, "Failed to load batsmen", Toast.LENGTH_SHORT).show();
                }
            });
        }
